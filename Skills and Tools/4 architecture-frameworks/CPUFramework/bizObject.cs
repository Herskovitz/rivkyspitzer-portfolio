...
namespace CPUFramework
{
    public class bizObject<T> : INotifyPropertyChanged where T : bizObject<T>, new()
    {
        ...
        public DataTable Load(int primarykeyvalue)
        {
            ...
            SQLUtility.SetParamaterValue(cmd, _primarykeyparamname, primarykeyvalue);
            dt = SQLUtility.GetDataTable(cmd);
            //set derived class properties from values in the database
            if (dt.Rows.Count > 0)
            {
                LoadProps(dt.Rows[0]);
            }
        ...
        public void Delete(int id)
        {
            ...
            SQLUtility.SetParamaterValue(cmd, _primarykeyparamname, id);
        ...
        public void Save()
        {
            //get sql command for the save sproc
            SqlCommand cmd = SQLUtility.GetSqlCommand(_updatesproc);
            //loop thru sproc params and find matching property names
            foreach (SqlParameter param in cmd.Parameters)
            {
                var prop = GetProp(param.ParameterName, true, false);
                //set the param value from the property values
                if (prop != null)
                {
        ...
        public void Save(DataTable datatable)
        {
            ...
            {
                throw new Exception($"Cannot call save  {_tablename}Save method because there are no rows in the table");
            }
        ...
        public List<T> GetList(bool includeblank = false)
        {
            ...
            SQLUtility.SetParamaterValue(cmd, "@IncludeBlank", includeblank);
        ...
        private PropertyInfo? GetProp(string propname, bool forread, bool forwrite)
        {
            ...
            PropertyInfo? prop = _properties.FirstOrDefault(p => p.Name.ToLower() == propname && ((forread == false || p.CanRead == true) && forwrite == false || p.CanWrite == true));
            return prop;
        }
