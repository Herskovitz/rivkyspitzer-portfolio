...
*/
const random_surcharge = function (itemname, price) { return price + Math.floor(Math.random() * 100); };
const fifty_percent_discount = (itemname, price) => price / 2;

function calculatePrice(itemname, price, f) {
    console.log(`Base price for ${itemname} is ${price} changed to ${f(itemname, price)}. Calculated with ${f.name}`);
}

function discount_based_on_name(itemname, price) {
    let newPrice;
    itemname.toLowerCase().includes("e") || itemname.toLowerCase().includes("a") ? newPrice = price - (price * .10) : newPrice = price - (price * .20);
    return newPrice
}


calculatePrice("Kn95 Mask", 10, discount_based_on_name);
calculatePrice("Book", 50, random_surcharge);
calculatePrice("Computer", 2425, fifty_percent_discount);
